...
/**
 * Check whether `name` refers to a node registered as a Claude Code subagent_type.
 *
 * Both `agent` and `group` nodes scaffold a `.claude/agents/<name>/AGENT.md`
 * file, which is what Claude Code registers as a subagent_type. Action, gate,
 * human, and orchestrator nodes are NOT registered and therefore CANNOT be
 * used as SubagentStop matchers.
 */
function isRegisteredSubagentType(ast: TopologyAST, name: string | undefined): boolean {
...
/**
 * Generate an observability-aware hook stub (Changes 4+5).
 *
 * For SubagentStop/Stop hooks: extract agent_type from the Claude Code stdin
 * JSON payload and export as AGENT_TYPE, then append a metrics.jsonl entry.
 * For other events: emit a simple TODO stub.
 */
function observabilityStub(hookName: string, eventName: string, matcherHint: string): string {
  const isFinishEvent = eventName === "SubagentStop" || eventName === "Stop";
  if (isFinishEvent) {
    return [
      "#!/usr/bin/env bash",
      `# Auto-generated by agentopology scaffold on ${new Date().toISOString().slice(0, 10)}`,
      `# Hook: ${hookName} · Event: ${eventName} · Matcher: ${matcherHint || "*"}`,
      "# Replace this stub with your implementation.",
      "# Stdin JSON payload from Claude Code is available on FD 0.",
      "set -euo pipefail",
      "",
      "PAYLOAD=$(cat)",
      ...
      "",
      `echo "{\\\"ts\\\":\\\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\\\",\\\"event\\\":\\\"${eventName}\\\",\\\"agent\\\":\\\"$AGENT_TYPE\\\",\\\"session\\\":\\\"$SESSION_ID\\\",\\\"hook\\\":\\\"${hookName}\\\"}" >> .claude/memory/metrics.jsonl`,
      "",
    ].join("\n");
  }
  return [
    ...
    'AGENT_TYPE=$(echo "$PAYLOAD" | jq -r \'.agent_type // "unknown"\')',
    "",
    `echo "TODO: implement ${hookName} hook (event: ${eventName}, agent: $AGENT_TYPE)"`,
    "",
  ].join("\n");
}

/** Generate hook script stubs for every hook's `run` field (Changes 1, 4, 5). */
function generateHookScripts(ast: TopologyAST): GeneratedFile[] {
  ...
  for (const hook of ast.hooks) {
    ...
    files.push({
      path: filePath,
      content: observabilityStub(hook.name, hook.on, hook.matcher ?? ""),
      category: "script",
      executable: true,
    });
  }
...
function generateSettings(ast: TopologyAST): GeneratedFile | null {
  ...
  //
  // SubagentStop fires when a subagent finishes. Its `matcher` field is matched
  // against `agent_type` (the registered subagent name from .claude/agents/),
  // NOT against arbitrary node ids. So a SubagentStop hook only fires when its
  // matcher names a real agent node — human, action, and orchestrator nodes are
  // NOT registered as subagent_types and their matchers would silently never
  // fire.
  //
  // SubagentStop with exit code 2 (or `decision: "block"`) DOES block: per the
  // Claude Code hooks reference, exit 2 "prevents subagent from stopping" and
  // surfaces stderr to Claude. That is the strongest enforcement available for
  // a gate that runs after an agent completes.
  //
  // For gates whose `after` target is not an agent (e.g. a human checkpoint, an
    ...
    if (!isRegisteredSubagentType(ast, gate.after)) {
      ...
      console.warn(
        `[claude-code] Gate "${gate.id}" has ${targetDesc} — SubagentStop hook not emitted ` +
          ...
          `invoke it from your orchestrator playbook or a slash command at the right step.`,
      );
      continue;
    }
