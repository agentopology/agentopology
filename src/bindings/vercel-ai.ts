...
function generateMetering(): string {
  return `/**
...
export class Meter {
  ...
  constructor(
    private outputPath: string = "./metrics.jsonl",
    private format: string = "jsonl",
  ) {}
...
${edgeRoutes.join("\n")}
  ...
  entryPoints: [${entryPoints.map((a) => `"${a.id}"`).join(", ")}],
  memory,
${observabilityConfig}
${checkpointConfig}
};
...
function scaffold(ast: TopologyAST): GeneratedFile[] {
  ...
  files.push({ path: "src/client.ts", content: generateClient(ast) });
  ...
  files.push({ path: "src/schemas.ts", content: generateSchemas(ast) });
  ...
  files.push({ path: "src/circuit-breaker.ts", content: generateCircuitBreaker() });
  files.push({ path: "src/permissions.ts", content: generatePermissions() });
  files.push({ path: "src/metering.ts", content: generateMetering() });
