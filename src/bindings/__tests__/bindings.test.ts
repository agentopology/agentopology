...
describe("claude-code binding", () => {
  ...
  it("gate stub contains the AGENTOPOLOGY_STUB marker (#4)", () => {
    // The gate's `run` script is a stub generated by shellStub() in the
    // shared lib. It must carry the marker so tooling can detect it.
    const stub = files.find((f) => f.path.endsWith("/scripts/lint.sh"));
    expect(stub).toBeDefined();
    expect(stub!.content).toContain(STUB_MARKER);
    expect(isStubContent(stub!.content)).toBe(true);
  });
});
...
// shellStub() util — which embeds the AGENTOPOLOGY_STUB marker. This test
// loops over every binding and verifies that at least one stub file in its
// scaffold output is detectable by isStubContent().
// ---------------------------------------------------------------------------

describe("Stub marker uniformity across bindings (#4)", () => {
  // The baseline TOPOLOGY_SOURCE at the top of this file has gate
  // `quality-check` whose `run: "scripts/lint.sh"` becomes a stub. Bindings
  // that scaffold gates (5 of 7) must produce a stub for it. The two that
  // don't (copilot-cli, cursor) have no gate-script generation and are
  // excluded.
  const stubProducers = ["claude-code", "codex", "gemini-cli", "kiro", "openclaw"];

  for (const bindingName of stubProducers) {
    it(`${bindingName} emits at least one stub with the AGENTOPOLOGY_STUB marker`, () => {
      const files = scaffoldBinding(bindingName);
      const stubs = files.filter((f) => isStubContent(f.content));
      expect(stubs.length, `${bindingName} should emit at least one stub`).toBeGreaterThan(0);
      // Every stub must contain the literal marker string (pinned by the
      // STUB_MARKER constant — see lib/stub.ts).
      for (const s of stubs) {
        expect(s.content).toContain(STUB_MARKER);
      }
    });
  }

  it("copilot-cli produces NO stubs (no shell-script scaffolds)", () => {
    // Sanity check — confirms the two non-stub-producing bindings stay
