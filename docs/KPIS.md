...
| L1.2 | **Zero silent drops** — a construct the parser ignores must produce an error | grep the parser for fields read but never stored; V-rule coverage | ⚠️ **1 known**: fields must be newline-separated and the grammar never says so (`parseFields` is line-based, so `{ model: sonnet retry: 3 }` makes `model` swallow the rest) | 0 known |
| L1.3 | **Spec matches implementation** | `src/parser/__tests__/spec-sync.test.ts` — compares the spec table against the `validate()` registry | ✅ **89 / 89**, contiguous V1-V89, guarded by 4 tests | stays equal |
...
| L2.2 | Test count and pass rate | `npx vitest run` | ✅ **1426 / 1426** | 100%, count grows with surface |
...
| L2.6 | Docs drift | `CLAUDE.md` claims vs measured | ✅ refreshed: **1426 tests / 89 rules / 10 examples / 5 exporters**. Was 1141 / 82 / 5 / 2. | refreshed at each release |
...
```
  LANGUAGE     ████████░░  5 of 6 green   — 1 amber: undocumented line-based fields
